#!/usr/bin/env python
"""
scripts/build_linkedin_assets.py
Generate LinkedIn-ready chart, table image, and post draft from sweep results.
Week-6 deliverable: LinkedIn assets (Step 4).
"""
...
def build_main_chart(df: pd.DataFrame) -> None:
    ...
    out = LINKEDIN_DIR / "main_chart.png"
...
def build_table_image(matrix: pd.DataFrame, regimes: dict) -> None:
    ...
    out = LINKEDIN_DIR / "strategic_regimes_table.png"
    fig.savefig(out, dpi=100, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"Wrote {out} ({out.stat().st_size:,} bytes)")


def build_post_draft(df: pd.DataFrame, matrix: pd.DataFrame) -> None:
    """Write the LinkedIn post draft."""
...
"""
    ...
    out = LINKEDIN_DIR / "post_draft.md"
    out.write_text(post)
    print(f"Wrote {out}")
...
def main() -> None:
    ...
    build_post_draft(df, matrix)
    print("\nAll LinkedIn assets generated in linkedin/")
